...
PetscErrorCode StokesSchurComplementIterationWrapper(PC pc, Vec x, Vec y) {
  StokesSchurComplementPreconditioning *shellPtr;
  PCShellGetContext(pc, (void **)&shellPtr);

  return shellPtr->ApplyPreconditioningIteration(x, y);
}

StokesSchurComplementPreconditioning::StokesSchurComplementPreconditioning()
...
PetscErrorCode
StokesSchurComplementPreconditioning::ApplyPreconditioningIteration(Vec x,
                                                                    Vec y) {
  PetscReal *a, *b;
  VecGetArray(x, &a);
  for (PetscInt i = 0; i < blockN_; i++) {
  ...
  auto &a01 = *(std::static_pointer_cast<PetscMatrix>(
      stokesLinearSystemsPtr_->GetMatrix(0, 1)));
  auto &a10 = *(std::static_pointer_cast<PetscMatrix>(
      stokesLinearSystemsPtr_->GetMatrix(1, 0)));

  KSPSolve(a00Ksp_, rhsVector_[0], lhsVector_[0]);
  stokesLinearSystemsPtr_->ConstantVec(lhsVector_[0]);
...
void StokesSchurComplementPreconditioning::AddLinearSystem(
    std::shared_ptr<StokesMatrix> mat) {
  stokesLinearSystemsPtr_ = mat;

  // prepare schur complement matrix
  Mat B, C;

  auto &a00 = *(std::static_pointer_cast<PetscMatrix>(
      stokesLinearSystemsPtr_->GetMatrix(0, 0)));
  auto &a01 = *(std::static_pointer_cast<PetscMatrix>(
      stokesLinearSystemsPtr_->GetMatrix(0, 1)));
  auto &a10 = *(std::static_pointer_cast<PetscMatrix>(
      stokesLinearSystemsPtr_->GetMatrix(1, 0)));
  auto &a11 = *(std::static_pointer_cast<PetscMatrix>(
      stokesLinearSystemsPtr_->GetMatrix(1, 1)));
  ...
  for (PetscInt i = 0; i < blockM_; i++) {
    PetscInt tLocalRow = stokesLinearSystemsPtr_->GetMatrix(i, 0)->GetRowSize();
    localRow += tLocalRow;
  ...
  for (PetscInt i = 0; i < blockN_; i++) {
    PetscInt tLocalCol = stokesLinearSystemsPtr_->GetMatrix(0, i)->GetColSize();
    localCol += tLocalCol;
  ...
  KSPSetTolerances(a00Ksp_, 1e-1, 1e-50, 1e20, 1);
