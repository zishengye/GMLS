...
PetscErrorCode HypreLUShellPCSetUp(PC pc, stokes_multilevel *multi, Vec x,
  ...
  shell->refinement_level = shell->multi->get_current_refinement_level();
...
PetscErrorCode HypreLUShellPCApplyAdaptive(PC pc, Vec x, Vec y) {
  ...
  KSPSolve(shell->multi->get_field_base()->get_reference(), x, y);
  ...
  //       KSPGetResidualNorm(
  //           shell->multi->get_colloid_relaxation(i)->get_reference(),
  //           &rnorm);
  //       VecNorm(shell->multi->get_x_colloid_list()[i]->get_reference(),
  //       NORM_2,
  //               &bnorm);
  ...
  //                       &reason);
  // KSPGetIterationNumber(shell->multi->get_field_base()->get_reference(),
  // &its); if (reason < 0) {
  //   PetscReal rnorm, bnorm;
  //   KSPGetResidualNorm(shell->multi->get_field_base()->get_reference(),
  //   &rnorm); VecNorm(shell->multi->get_b_field_list()[0]->get_reference(),
  //   NORM_2,
  //           &bnorm);

  //   PetscReal *a;
  //   PetscInt na;
  //   KSPGetResidualHistory(shell->multi->get_field_base()->get_reference(),
  //   &a,
  //                         &na);
  ...
  //       PetscPrintf(PETSC_COMM_WORLD,
  //                   "colloid smooth sweep up convergence reason: %d, number
  //                   of " "iterations: %d, last " "residual norm: %f, rhs
  //                   norm: %f\n", reason, its, rnorm, bnorm);
  //     }
