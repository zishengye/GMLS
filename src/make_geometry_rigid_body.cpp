...
void GMLS_Solver::InitRigidBody() {
  ...
  // rigidBodyPosition[2][1] = 1.0;
  // rigidBodyPosition[2][2] = 1.0;
  // rigidBodySize[2] = 1.0;

  // rigidBodyPosition[3][0] = 0.0;
  // rigidBodyPosition[3][1] = -1.0;
  // rigidBodyPosition[3][2] = -1.5;
  // rigidBodySize[3] = 1.0;
...
int GMLS_Solver::IsInRigidBody(vec3 &pos, double h) {
  ...
  for (size_t i = 0; i < rigidBodyCoord.size(); i++) {
    ...
    if (dis.mag() < rigidBodySize[i] - 0.5 * __particleSize0[0]) {
      return -1;
    } else if ((dis.mag() + 1e-15) < (rigidBodySize[i] + 0.6 * h)) {
      return i;
    }
  }
