...
//             int destination = (__nI + offsetX[i]) + (__nJ + offsetY[j]) *
//             __nX; if (destination >= 0 && destination != __myID &&
...
//   for (int i = 0; i < neighborNum; i++) {
//     if (neighborFlag[i] == true) {
//       MPI_Isend(sendDataBlock[i].data(), sendCount[i] * unitLength,
//       MPI_DOUBLE,
//                 destinationIndex[i], 0, MPI_COMM_WORLD,
//                 send_request.data() + count);
//       MPI_Irecv(tempRecvData.data() + recvOffset[i] * unitLength,
//                 recvCount[i] * unitLength, MPI_DOUBLE, destinationIndex[i],
//                 0, MPI_COMM_WORLD, recv_request.data() + count);
//       count++;
//     }
//   }
